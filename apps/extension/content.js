...
(function () {
  ...
  // ── Decode + Render ─────────────────────────────────────────
  chrome.storage.local.get(['geminiApiKey', 'chainmergeApiUrl'], async ({ geminiApiKey, chainmergeApiUrl }) => {
    ...
    const result = await new Promise((resolve) => {
      chrome.runtime.sendMessage(
        { type: 'CM_DECODE', chain: explorerInfo.chain, hash: explorerInfo.hash, apiUrl },
        resolve
      );
    });
