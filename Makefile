.PHONY: help test test-core test-api build-web run-api make-api run-web dev docker-up docker-down docker-logs
...
run-api:
	...
	if [ -z "$$POLKADOT_SUBSCAN_API_KEY" ]; then \
		echo "WARN: POLKADOT_SUBSCAN_API_KEY is not set (Polkadot decode may fail)."; \
	fi; \
